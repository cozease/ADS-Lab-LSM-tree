#include "test.h"

#include <chrono>
#include <cstdint>
#include <iostream>
#include <string>

class CorrectnessTest : public Test {
private:
    const uint64_t SIMPLE_TEST_MAX = 512;
    const uint64_t LARGE_TEST_MAX  = 1024 * 64;

    void insert_test(uint64_t max) {
        uint64_t i;
        auto put_start = std::chrono::high_resolution_clock::now();
        // 计算平均put时间，单位是微秒
        long long int sum = 0;
        for (i = 0; i < max; ++i) {
            auto start = std::chrono::high_resolution_clock::now();
            store.put(i, std::string(i + 1, 's'));
            auto end = std::chrono::high_resolution_clock::now();
            sum += std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        }
        std::cout << "average put time: " << (double)sum / (double)max << "us" << std::endl;
        auto put_end = std::chrono::high_resolution_clock::now();
        std::cout << "put time: " << std::chrono::duration_cast<std::chrono::milliseconds>(put_end - put_start).count() << "ms" << std::endl;

        auto get_start = std::chrono::high_resolution_clock::now();
        sum = 0;
        for (i = 0; i < max; ++i) {
            auto start = std::chrono::high_resolution_clock::now();
            EXPECT(std::string(i + 1, 's'), store.get(i));
            auto end = std::chrono::high_resolution_clock::now();
            sum += std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        }
        std::cout << "average get time: " << (double)sum / (double)max << "us" << std::endl;
        auto get_end = std::chrono::high_resolution_clock::now();
        std::cout << "get time: " << std::chrono::duration_cast<std::chrono::milliseconds>(get_end - get_start).count() << "ms" << std::endl;

        phase();
    }

    void delete_test(uint64_t max) {
        uint64_t i;
        // Test a single key
        EXPECT(not_found, store.get(1));
        store.put(1, "SE");
        EXPECT("SE", store.get(1));
        EXPECT(true, store.del(1));
        EXPECT(not_found, store.get(1));
        EXPECT(false, store.del(1));

        phase();

        // Test multiple key-value pairs
        for (i = 0; i < max; ++i) {
            store.put(i, std::string(i + 1, 's'));
            EXPECT(std::string(i + 1, 's'), store.get(i));
        }
        phase();

        // Test deletions
        auto del_start = std::chrono::high_resolution_clock::now();
        long long int sum = 0;
        for (i = 0; i < max; i += 2) {
            auto start = std::chrono::high_resolution_clock::now();
            EXPECT(true, store.del(i));
            auto end = std::chrono::high_resolution_clock::now();
            sum += std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        }
        std::cout << "average delete time: " << (double)sum / (double)max / 2.0 << "us" << std::endl;
        auto del_end = std::chrono::high_resolution_clock::now();
        std::cout << "del time: " << std::chrono::duration_cast<std::chrono::milliseconds>(del_end - del_start).count() << "ms" << std::endl;

        for (i = 0; i < max; ++i) {
            switch (i & 3) {
            case 0: // 4k
                EXPECT(not_found, store.get(i));
                store.put(i, std::string(i + 1, 't'));
                break;
            case 1: // 4k+1
                EXPECT(std::string(i + 1, 's'), store.get(i));
                store.put(i, std::string(i + 1, 't'));
                break;
            case 2: // 4k+2
                EXPECT(not_found, store.get(i));
                break;
            case 3: // 4k+3
                EXPECT(std::string(i + 1, 's'), store.get(i));
                break;
            }
        }

        phase();

        report();
    }

    void regular_test(uint64_t max) {
        uint64_t i;

        // Test a single key
        EXPECT(not_found, store.get(1));
        store.put(1, "SE");
        EXPECT("SE", store.get(1));
        EXPECT(true, store.del(1));
        EXPECT(not_found, store.get(1));
        EXPECT(false, store.del(1));

        phase();

        // Test multiple key-value pairs
        for (i = 0; i < max; ++i) {
            store.put(i, std::string(i + 1, 's'));
            EXPECT(std::string(i + 1, 's'), store.get(i));
        }
        phase();

        // Test after all insertions
        for (i = 0; i < max; ++i)
            EXPECT(std::string(i + 1, 's'), store.get(i));
        phase();

        // Test scan
        std::list<std::pair<uint64_t, std::string>> list_ans;
        std::list<std::pair<uint64_t, std::string>> list_stu;

        for (i = 0; i < max / 2; ++i) {
            list_ans.emplace_back(std::make_pair(i, std::string(i + 1, 's')));
        }

        store.scan(0, max / 2 - 1, list_stu);
        EXPECT(list_ans.size(), list_stu.size());

        auto ap = list_ans.begin();
        auto sp = list_stu.begin();
        while (ap != list_ans.end()) {
            if (sp == list_stu.end()) {
                EXPECT((*ap).first, -1);
                EXPECT((*ap).second, not_found);
                ap++;
            } else {
                EXPECT((*ap).first, (*sp).first);
                EXPECT((*ap).second, (*sp).second);
                ap++;
                sp++;
            }
        }

        phase();

        // Test deletions
        for (i = 0; i < max; i += 2)
            EXPECT(true, store.del(i));

        for (i = 0; i < max; ++i)
            EXPECT((i & 1) ? std::string(i + 1, 's') : not_found, store.get(i));

        for (i = 1; i < max; ++i)
            EXPECT(i & 1, store.del(i));

        phase();

        report();
    }

public:
    CorrectnessTest(const std::string &dir, bool v = true) : Test(dir, v) {}

    void start_test(void *args = NULL) override {
        std::cout << "KVStore Correctness Test" << std::endl;

        // store.reset();

        // std::cout << "[Simple Test]" << std::endl;
        // regular_test(SIMPLE_TEST_MAX);

        store.reset();

        std::cout << "[Large Test]" << std::endl;
        regular_test(128);

        // store.reset();
        // std::cout << "[Insert Test]" << std::endl;
        // insert_test(1024 * 64);

        // store.reset();
        // std::cout << "[delete test]" << std::endl;
        // delete_test(1024 * 64);
    }
};

int main(int argc, char *argv[]) {
    bool verbose = (argc == 2 && std::string(argv[1]) == "-v");

    std::cout << "Usage: " << argv[0] << " [-v]" << std::endl;
    std::cout << "  -v: print extra info for failed tests [currently ";
    std::cout << (verbose ? "ON" : "OFF") << "]" << std::endl;
    std::cout << std::endl;
    std::cout.flush();

    CorrectnessTest test("./data", verbose);

    test.start_test();

    return 0;
}
